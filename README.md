...
d:\apptuoicaytudong\
├── firmware\
│   └── firmware.ino          # Mã nguồn nạp cho vi điều khiển ESP32
...
2. Tạo một Project mới (ví dụ: `Smartgarden`).
...
## 🔌 Hướng Dẫn Nạp Chương Trình Cho ESP32 (Firmware)

### 1. Sơ đồ nối dây linh kiện
| Linh kiện | Chân trên ESP32 DevKit V1 | Ghi chú |
| :--- | :--- | :--- |
| **Cảm biến độ ẩm đất** | **GPIO 36 / VP** (ADC) | Đọc giá trị tương tự (ADC thô từ 0 đến 4095 trên ESP32) |
| **Relay điều khiển Bơm** | **GPIO 26** | Kích mức HIGH để bật bơm (Có thể thay đổi trong code nếu Relay kích mức LOW) |
| **Nút nhấn Reset WiFi** | **GPIO 27** | Một chân nối GPIO 27, chân còn lại nối GND (Nút nhấn kéo xuống GND) |
| **Đèn LED Trạng thái** | **GPIO 2 (Built-in)** | Đèn LED tích hợp sẵn trên mạch (nháy báo trạng thái kết nối & bơm) |
...
- Cập nhật thông tin cấu hình Firebase của bạn ở phần khai báo:
  ...
  ```
- Kết nối mạch ESP32 với máy tính, chọn đúng Cổng COM (Port) và Board tương ứng (ví dụ: `DOIT ESP32 DEVKIT V1` hoặc `ESP32 Dev Module`).
...
- ESP32 sẽ tự động phát ra một mạng WiFi tên là **`HeThongTuoi_CapNhatWiFi`** (không mật khẩu).
...
- ESP32 sẽ tự động lưu mật khẩu, tắt đèn LED mờ và kết nối Internet để đồng bộ với Firebase.

### 5. Cách reset mạng WiFi (Nút nhấn 5 giây)
- Khi muốn thay đổi sang một mạng WiFi mới, trong lúc mạch đang hoạt động bình thường, hãy **nhấn và giữ nút nhấn ở chân GPIO 27 trong 5 giây**.
...
4. Trang web sẽ tự động lưu cấu hình vào bộ nhớ trình duyệt (`localStorage`) và tự động kết nối trực tiếp đến Firebase của bạn. Từ lúc này, mọi tương tác của cảm biến và nút gạt sẽ đồng bộ thời gian thực:
   ...
   - **Chế độ tự động/Thủ công** đồng bộ 2 chiều (Chuyển chế độ trên App sẽ làm thay đổi hành vi xử lý của ESP32 ngay lập tức).
   - **Nút bật bơm thủ công** chỉ kích hoạt khi bạn tắt chế độ tự động (chuyển sang Thủ công).
   - **Thanh trượt Ngưỡng tưới** cho phép bạn thay đổi ngưỡng khô mà ESP32 dùng để quyết định bật bơm ở chế độ Tự động.
